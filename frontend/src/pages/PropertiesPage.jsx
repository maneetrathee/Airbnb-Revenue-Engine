...
import {
  Building2, Plus, Pencil, Trash2, Loader2, X,
  BedDouble, MapPin, Home, ToggleLeft, ToggleRight,
  PoundSterling, ChevronDown, Shield, Globe, AlertCircle,
...
const Toast = ({ toast }) => {
  ...
  return (
    <div className={`fixed top-6 right-6 z-50 flex items-center gap-3 px-5 py-3 rounded-xl shadow-xl text-sm font-medium border animate-in slide-in-from-top-3 duration-300 ${
      toast.type === "success"
...
const SyncDrawer = ({ property, globalSettings, onClose, onSave }) => {
  const [useGlobal, setUseGlobal] = useState(property.sync_settings?.use_global ?? true);
  const [enabled, setEnabled]     = useState(property.sync_settings?.enabled ?? false);
  const [minPrice, setMinPrice]   = useState(property.sync_settings?.min_price ?? 30);
  const [maxPrice, setMaxPrice]   = useState(property.sync_settings?.max_price ?? 500);
  const [saving, setSaving]       = useState(false);

  const handleSave = async () => {
    setSaving(true);
    await onSave(property.id, { enabled, min_price: minPrice, max_price: maxPrice, use_global: useGlobal });
    setSaving(false);
    onClose();
  };
    ...
    <div className="fixed inset-0 z-40 flex">
      ...
      <div className="w-96 bg-white shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          {/* Use global toggle */}
          <div className={`p-4 rounded-xl border-2 transition-all ${useGlobal ? "border-brand bg-rose-50" : "border-gray-200 bg-gray-50"}`}>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <Globe size={18} className={useGlobal ? "text-brand" : "text-gray-400"} />
                <div>
                  <p className="font-bold text-gray-900 text-sm">Use Global Defaults</p>
                  <p className="text-xs text-gray-500">Inherit settings from global config</p>
                </div>
              </div>
            ...
            {useGlobal && (
              <div className="mt-3 pt-3 border-t border-rose-200 text-xs text-rose-700 space-y-1">
                <p>• Auto-sync: <strong>{globalSettings?.enabled ? "ON" : "OFF"}</strong></p>
                <p>• Price range: <strong>£{globalSettings?.min_price} – £{globalSettings?.max_price}</strong></p>
              </div>
            )}
          </div>

          {/* Per-property overrides — only shown when not using global */}
          {!useGlobal && (
            <div className="space-y-5 animate-in fade-in duration-200">
              ...
              <div className="flex items-center justify-between p-4 bg-gray-50 rounded-xl">
                <div>
                  ...
                  <p className="text-xs text-gray-500">Override for this property only</p>
                </div>
              ...
              <div>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <p className="font-bold text-gray-900 text-sm">Price Guardrails</p>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-xs font-bold text-gray-500 uppercase tracking-wider block mb-1">Min £</label>
                    <div className="relative">
                      <PoundSterling size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
                      <input type="number" value={minPrice} min={10}
                  ...
                  <div>
                    <label className="text-xs font-bold text-gray-500 uppercase tracking-wider block mb-1">Max £</label>
                    <div className="relative">
                      <PoundSterling size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
                      <input type="number" value={maxPrice} min={10}
...
const PropertyModal = ({ property, neighborhoods, onClose, onSave }) => {
  ...
  // Use placeholder if no photo_url
  const previewSrc = form.photo_url || PLACEHOLDER_PHOTOS[previewIdx % PLACEHOLDER_PHOTOS.length];
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto animate-in zoom-in-95 duration-200">
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="relative h-40 rounded-xl overflow-hidden bg-gray-100 group">
            <img src={previewSrc} alt="Property" className="w-full h-full object-cover" />
          ...
          {/* Photo URL */}
          <div>
            <label className="text-xs font-bold text-gray-500 uppercase tracking-wider block mb-1.5">Photo URL</label>
            ...
            {!form.photo_url && (
              <div className="flex gap-2 mt-2">
                ...
                <span className="text-xs text-gray-400 self-center ml-1">Pick placeholder</span>
              </div>
            )}
          </div>
          ...
          {/* Name */}
          <div>
            <label className="text-xs font-bold text-gray-500 uppercase tracking-wider block mb-1.5">Property Name *</label>
          ...
          {/* Neighborhood */}
          <div>
            <label className="text-xs font-bold text-gray-500 uppercase tracking-wider block mb-1.5">Neighborhood</label>
            <div className="relative">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-xs font-bold text-gray-500 uppercase tracking-wider block mb-1.5">Room Type</label>
              <div className="relative">
            ...
            <div>
              ...
              <input type="number" value={form.bedrooms} min={1} max={20}
        ...
        <div className="p-6 border-t border-gray-100 flex gap-3 sticky bottom-0 bg-white">
          <button onClick={onClose} className="flex-1 py-2.5 border border-gray-200 text-gray-700 font-bold rounded-xl hover:bg-gray-50 transition-all">
            Cancel
          </button>
...
// ── Property card ─────────────────────────────────────────────────────────────
const PropertyCard = ({ property, onEdit, onDelete, onSync }) => {
  const sync = property.sync_settings;
  const isActive = sync?.enabled && !sync?.use_global || (sync?.use_global);
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden hover:shadow-md transition-shadow group">
      ...
      <div className="relative h-44 overflow-hidden bg-gray-100">
        <img
          src={property.photo_url || `https://images.unsplash.com/photo-1522708323590-d24dbb6b0267?w=400&q=80`}
          alt={property.name}
        ...
        <div className={`absolute top-3 right-3 px-2.5 py-1 rounded-full text-xs font-bold ${
          ...
          {sync?.use_global ? "Global Sync" : sync?.enabled ? "Auto-Sync ON" : "Manual"}
        </div>
      </div>
      ...
      <div className="p-5">
        ...
        {!sync?.use_global && (
          <div className="flex items-center gap-1.5 text-xs text-gray-400 mb-4 bg-gray-50 rounded-lg px-3 py-2">
            ...
            <span>£{sync?.min_price} – £{sync?.max_price} guardrail</span>
          </div>
        )}
...
export default function PropertiesPage() {
  ...
  useEffect(() => {
    ...
    fetch(`${BASE_URL}/api/v1/sync/settings/${userId}`)
      .then(r => r.json()).then(setGlobalSettings);
  }, [userId]);
  ...
  const handleSaveProperty = async (form, propertyId) => {
    ...
    if (data.success) {
      showToast("success", propertyId ? "Property updated!" : "Property added!");
      fetchProperties();
    } else {
  ...
  const handleDelete = async (propertyId) => {
    ...
    const res = await fetch(`${BASE_URL}/api/v1/properties/${userId}/${propertyId}`, { method: "DELETE" });
    const data = await res.json();
    if (data.success) {
  ...
  const handleSaveSyncSettings = async (propertyId, settings) => {
    const res = await fetch(`${BASE_URL}/api/v1/properties/${propertyId}/sync-settings`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 mt-2">
            Manage your portfolio. Each property can have independent sync settings.
          </p>
        </div>
        <button
        ...
        <div className="flex items-center gap-4 p-4 bg-gray-900 text-white rounded-2xl">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-gray-400 mt-0.5">
              Auto-sync: <strong className={globalSettings.enabled ? "text-emerald-400" : "text-gray-400"}>
                {globalSettings.enabled ? "ON" : "OFF"}
              </strong>
              {" · "}Price range: <strong className="text-white">£{globalSettings.min_price} – £{globalSettings.max_price}</strong>
              {" · "}Area: <strong className="text-white">{globalSettings.neighborhood || "Not set"}</strong>
            </p>
          </div>
          <span className="text-xs text-gray-500">Applied to all properties using global settings</span>
        </div>
      )}
      ...
      {!loading && properties.length === 0 && (
        <div className="text-center py-20 bg-white rounded-2xl border border-dashed border-gray-300">
          ...
          <p className="text-gray-500 text-sm mb-6">Add your first property to start managing pricing.</p>
          <button
            onClick={() => { setEditingProp(null); setShowModal(true); }}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {properties.map(prop => (
            <PropertyCard
              key={prop.id}
              property={prop}
              onEdit={p => { setEditingProp(p); setShowModal(true); }}
